...
int count(struct  Rec ** node)//keeps count
{
	int count=0;
	while(*node){
		if(count < 100){
...
struct  Rec * split(struct  Rec ** node, int count)//splits the linked list
{
	if( count % 2 == 0){
	--count;
	}
	if(count == 3){
	struct  Rec ** temp = &((*node)->next);
	struct  Rec * headB = (*temp)->next;
	(*temp)->next = NULL;
	return headB;
	}
	int x; struct  Rec ** temp = node;
	for(x = count/2; x > 0; x --){
	temp = &((*temp)->next);
	}
	struct  Rec * headB = (*temp)->next;
...
struct  Rec **
mergeHelper(struct  Rec **node,struct  Rec ** headB,int sublistB,int headerNum)
...
struct  Rec ** merge(struct  Rec ** node, struct  Rec ** headB, int headerNum)
...
struct  Rec **
mergesortHelper(struct  Rec *temp,struct  Rec ** node)
...
struct  Rec **
mergesort(struct  Rec ** node, int headerNum)
 {//Does the mergesort!
	 int size = count(node);
	 if(size > 2){
		struct  Rec * headB; struct  Rec * fhead;
		headB = split(node,size);
		struct  Rec ** secondHalf = &headB;
